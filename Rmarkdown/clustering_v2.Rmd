...
for(i in 1:300){
 #ran = sample(1:15000,5)
  ...
  #fraction of genes significant out of 100
  k2 = kmeans(bulk_subsampled,2,nstart = 100)
  genes_temp = rep(0,100)
  for(g in 1:100)
  {
     data = data.frame(as.factor(k2$cluster),bulk_subsampled[,g])
     colnames(data) = c('gr','gene')
    lm_out = summary(lm(gene~gr,data))
    genes_temp[g] = lm_out$coefficients[2,4]
    #if(lm_out$coefficients[2,4] < 0.05) genes = genes + 1
  }
  genes[i,1] = length(genes_temp[genes_temp<(0.05/100)])
