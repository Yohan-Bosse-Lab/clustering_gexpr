...
params:
  ...
  outputpath: 'C:/Users/renseb01/Documents/clustering_gexpr/results/Oct2025'
  repo: 'C:/Users/renseb01/Documents/clustering_gexpr'
---
...
clinical_bigtable = read.csv(file.path(params$repo,'../rnaseq/data/clinical_bigtable_v3.csv'), check.names = F)
 ...
 #3. get bulkRNAseq data
  ...
  #choose a specific k you want to analyse
  k = 3 #or k = 2
...
predictors = list(c('top5','SFTPC'),Bianchi2007,Kratz2012,Der2014,Wilkerson2012,Raz2008,Shukla2016,Wistuba2013,Peinado2022,Biswas2019)
names(predictors) = c('top5%','Bianchi 2007','Kratz 2012','Der 2014','Wilkerson 2012','Raz 2008','Shukla 2016','Wistuba 2013','Peinado-Serrano 2022','Biswas 2019')
sil_plots = list() 

#write results as table S1
tableS1 = data.frame(matrix(NA,nrow = 31, ncol= 9))
colnames(tableS1) = c('Bianchi 2007','Kratz 2012','Der 2014','Wilkerson 2012','Raz 2008','Shukla 2016','Wistuba 2013','Peinado-Serrano 2022','Biswas 2019')

for(j in 1:9){
  tableS1[1:length(predictors[[j+1]]),j] = predictors[[j+1]]
}

write.table(tableS1,file.path(params$repo,'data/tableS1.tsv'),sep = '\t',row.names = F,quote = F)
...
for(i in 1:10) {
  ...
  if(i >1){
   ...
   # if((i == 3) | (i == 5)) marker_based_signatures = kmeans(bulk_vst_variable_tumeur,3,nstart = 100)
    ...
    if(i == 10) cluster_table$Biswas2019 =  as.factor(marker_based_signatures$cluster)
...
#largest size
clinical_cluster$`Tumor largest\ndimension (mm)` = NA
maxsize_quants = quantile(clinical_cluster$`Pathology_Tumor_size_largest size`)
clinical_cluster$`Tumor largest\ndimension (mm)`[clinical_cluster$`Pathology_Tumor_size_largest size` <= maxsize_quants[2]] = '<22'
clinical_cluster$`Tumor largest\ndimension (mm)`[(clinical_cluster$`Pathology_Tumor_size_largest size` > maxsize_quants[2]) & (clinical_cluster$`Pathology_Tumor_size_largest size` <= maxsize_quants[3])] = '22-27'
clinical_cluster$`Tumor largest\ndimension (mm)`[(clinical_cluster$`Pathology_Tumor_size_largest size` > maxsize_quants[3]) & (clinical_cluster$`Pathology_Tumor_size_largest size` <= maxsize_quants[4])] = '27-38'
clinical_cluster$`Tumor largest\ndimension (mm)`[clinical_cluster$`Pathology_Tumor_size_largest size` > maxsize_quants[4]] = '>38'
clinical_cluster$`Tumor largest\ndimension (mm)` = factor(clinical_cluster$`Tumor largest\ndimension (mm)`,levels = unique(clinical_cluster$`Tumor largest\ndimension (mm)`)[c(4,1,2,3)])
...
#10 year censoring
clinical_cluster$status[clinical_cluster$pfs>10] = 0 
...
###
clinical_cluster$`Pathology_Tumor_Solid %`[is.na(clinical_cluster$`Pathology_Tumor_Solid %`)] = 0 
clinical_cluster$`Pathology_Tumor_Micropapillary %`[is.na(clinical_cluster$`Pathology_Tumor_Micropapillary %`)] = 0
clinical_cluster$`Pathology_Tumor_Fused glands %`[is.na(clinical_cluster$`Pathology_Tumor_Fused glands %`)] = 0
clinical_cluster$`Pathology_Tumor_Cribriform %`[is.na(clinical_cluster$`Pathology_Tumor_Cribriform %`)] = 0
                 
                          
clinical_cluster$`Predominant\nfeature` = 'High (Solid | \nMpap | CGP)'

clinical_cluster$`Predominant\nfeature`[clinical_cluster$`Pathology_Tumor_Predominant feature` == 'Acinar' & ((clinical_cluster$`Pathology_Tumor_Solid %` < 20) | (clinical_cluster$`Pathology_Tumor_Micropapillary %` < 20))] = 'Intermediate\n(Acinar | Pap)'

clinical_cluster$`Predominant\nfeature`[clinical_cluster$`Pathology_Tumor_Predominant feature` == 'Papillary' & ((clinical_cluster$`Pathology_Tumor_Solid %` < 20) | (clinical_cluster$`Pathology_Tumor_Micropapillary %` < 20))] = 'Intermediate\n(Acinar | Pap)'

clinical_cluster$`Predominant\nfeature`[clinical_cluster$`Pathology_Tumor_Predominant feature` == 'Lepidic'] = 'Low (Lepidic)'
...
clinical_cluster$`Predominant\nfeature` = factor(clinical_cluster$`Predominant\nfeature`, levels = c('Low (Lepidic)','Intermediate\n(Acinar | Pap)','High (Solid | \nMpap | CGP)','Unknown\n(Mucinous)'))
...
pfits[[1]] = ggsurvplot(fit,pval = T,conf.int = F,legend.title = "k = 2",legend.labs = c('cluster 2 (low risk)','cluster 1 (high risk)'),xlab = c('Time (years)'),palette = palette[2:1] )$plot+ geom_vline(xintercept = 5,linetype = 'dashed')
...
factors = c('Tumor largest\ndimension (mm)','Grade','Predominant\nfeature','Stage','Pack year','Smoking\nstatus','Age','Sex','Pathology_Tumor_ADK-Mucinous',
            'Bianchi2007','Kratz2012','Der2014','Wilkerson2012','Wistuba2013','Shukla2016','Raz2008','Peinado2022','Biswas2019',paste('k',k,sep = ''))
...
#
gexpr_factors = c('top25% variable','Bianchi 2007','Der 2014','Kratz 2012','Wilkerson 2012','Wistuba 2013','Shukla 2016','Raz 2008','Peinado 2022','Biswas 2019')
hazard_summarised_clin = hazard_summarised[hazard_summarised$names %in% c('Smoking\nstatus','Sex','Age','Pack year'), ]
hazard_summarised_patho = hazard_summarised[hazard_summarised$names %in% c('Tumor largest\ndimension (mm)','Stage','Grade','Predominant\nfeature'), ]
...
rects_path = data.frame(ystart = c(0.5,4.5,7.5,11.5), yend = c(4.5,7.5,11.5,16.5), 
         col = c('a','b','a','b'))
...
upset_plot = upset(clinical_cluster_upset,colnames(clinical_cluster_upset)[5:7],
        ...
        list(
            ...
            'Intersection size'= list(ylab('Number of patients'),ggtitle('Patients overlap in high risk clusters'))
        )
    ))
```
...
clinical_cluster$survival [(clinical_cluster$k2 == tail(levels(clinical_cluster$k2),1)) &
                              (clinical_cluster$k3 == tail(levels(clinical_cluster$k3),1))
                               & (clinical_cluster$k4 == tail(levels(clinical_cluster$k4),1))] = 'high'

clinical_cluster$survival = factor(clinical_cluster$survival,levels = c('high','low'))
...
tumorlargestdim = clinical_cluster %>% 
  group_by(survival, `Tumor largest\ndimension (mm)`) %>% 
  summarise(len =length(`Tumor largest\ndimension (mm)`)) %>% 
  ggplot(aes(fill = `Tumor largest\ndimension (mm)`, x = len, y = survival)) + 
  geom_bar(position='fill', stat="identity") +
  ylab('Risk group') +
  xlab('Proportions') +
 # ggtitle('Percentage tumor') +
...
(( pfits[[1]] / pfits[[2]] / pfits[[3]]) + plot_layout(height = c(3,3,3)) | (
  ((upset_plot | venn_plot) + plot_layout(width = c(4,3)) ) / ((sex | smoke | patho) / (feature | grade | tumorlargestdim)))) + plot_layout(width = c(4,5))  + plot_annotation(tag_levels = list(c('A','B','C','D','E','','F','G','H','I','J','K')))

dev.off()

pdf(file.path(params$outputpath,paste0('FigureS3_perctumor.pdf')),width = 5,height = 5)
perctumor
dev.off()
...
#QC of percentage tumor
```{r percentagetumor}

percenttumor2 = ggplot(clinical_cluster) + 
  geom_boxplot(aes(y = `Pathology_Tumor_Percentage of Tumor`, x = k2,fill = k2 )) +
  ggtitle('K=2') +
  ylab('Percentage of tumor') +
  xlab('Cluster labels') +
  theme_bw() 


percenttumor3 = ggplot(clinical_cluster) + 
  geom_boxplot(aes(y = `Pathology_Tumor_Percentage of Tumor`, x = k3,fill = k3 )) +
  ggtitle('K=3') +
  ylab('Percentage of tumor') +
  xlab('Cluster labels') +
  theme_bw() 

percenttumor4 = ggplot(clinical_cluster) + 
  geom_boxplot(aes(y = `Pathology_Tumor_Percentage of Tumor`, x = k4,fill = k4 )) +
  ggtitle('K=4') +
  ylab('Percentage of tumor') +
  xlab('Cluster labels') +
  theme_bw() 

pdf(file.path(params$outputpath,paste0('FigureSup_percentge_tumor.pdf')),width = 10,height = 6)
percenttumor2 + percenttumor3 + percenttumor4 + plot_annotation(tag_levels = 'A')
dev.off()


```
  
#gene expression of high risk group.
```{r deseq, message= T,eval = F}
  txi = readRDS(file.path(params$repo,'../rnaseq/lord_kallisto/txi.rds'))
  #bulk = as.matrix(txi$abundance)

  bulk <- round(txi$counts)
  rownames(bulk) = sapply(strsplit(rownames(bulk), ".",fixed = T), "[",1)
  colnames(bulk) = sapply(strsplit(colnames(bulk), ".",fixed = T), "[",5)
  bulk_deseq_data = bulk[,grep('Tumeur',colnames(bulk))]
  colnames(bulk_deseq_data) = sapply(strsplit(colnames(bulk_deseq_data), "_",fixed = T), "[",3)
  
  #order the bulk rnaseq data
  bulk_deseq_data = bulk_deseq_data[,order(as.numeric(colnames(bulk_deseq_data)))]  
  
  #conversion from Ensembl -> SYMBOL
  conversion = biomaRt::select(org.Hs.eg.db, keys = rownames(bulk_deseq_data), columns = c("ENTREZID", "SYMBOL","ENSEMBL"), keytype = "ENSEMBL")
  conversion = conversion[!is.na(conversion$ENSEMBL),]
  conversion = conversion[!is.na(conversion$SYMBOL),]
  conversion = conversion[!duplicated(conversion$ENSEMBL), c(1,3)]
  conversion = conversion[!duplicated(conversion$SYMBOL),]
  temp = merge(bulk_deseq_data,conversion,by.x=0,by.y= 'ENSEMBL')
  bulk_deseq_data = temp[,-c(1,517)]
  rownames(bulk_deseq_data) = temp$SYMBOL
  
  #remove very low expression genes
  bulk_deseq_data_rmlow = bulk_deseq_data[rowMeans(bulk_deseq_data) > 10,]
  
  #subet the clinical data and order it according to record id.
  clinical_small = clinical_cluster[,colnames(clinical_cluster) %in% c('survival','Record ID')]
  clinical_small = clinical_small[order(as.numeric(clinical_small$`Record ID`)),]  
    
  #check it 
  all.equal(colnames(bulk_deseq_data_rmlow),clinical_small$`Record ID`)
  
  #deseq object
  dds = DESeqDataSetFromMatrix(bulk_deseq_data_rmlow,colData = clinical_small, ~ survival)
  dds = DESeq(dds)
  resultsNames(dds)
  
  res = results(dds) # low vs high
  res = res[order(res$pvalue),] #order by pval
  res = res[res$baseMean>1,] #remove very low expression values
  res = res[!is.na(res$padj),]
  
  
#plotting  
data = res
data$minuslogpadj = -log(res$padj,10)
data$genes = 'non-significant'
data$genes[data$padj<0.01 & abs(data$log2FoldChange)>1] = 'significant'

table(data$genes)

#
volcano_plot = ggplot(data,aes(x = log2FoldChange, y = minuslogpadj,color = genes)) + 
  geom_point(pch =1) +
  xlim(c(-4,4)) +
  ylim(c(-1,40)) +
  ggtitle('Differential Gene Expression (High vs Low risk group)') +
...
```{r GO analysis, message= T,eval = F}
library(topGO)
library(biomaRt)
library(org.Hs.eg.db)
...
comparison = c('High vs Low')
  ...
  gene_list[gene_list=='non-significant'] = 1
  gene_list[gene_list=='significant'] = 0
  ...
  go_final <- GenTable(GOdata, resultFis, topNodes = 10)
  go_final$comparison = 'high low'
  go_final$oe = round(go_final$Significant/go_final$Expected,2)
  go_final$'-log (pvalue)' = -log(as.numeric(gsub('< ','',go_final$result1)),10)
  go_final = go_final[order(go_final$oe),]
  go_final$Term[go_final$Term == 'production of molecular mediator of immu...'] = 'production of molecular mediator\nof immune response'
  go_final$Term = factor(go_final$Term,levels = go_final$Term) 
  
  #go plots $###
go_plot = ggplot(data = go_final,aes(y = Term, x = oe, fill= `-log (pvalue)`)) + 
  geom_bar(stat="identity") +
  ylab('Top 10 significant GO terms (BP)') +
  xlab('Observed / Expected Number of Genes') +
  scale_fill_continuous(bquote(-log[10]~p[value])) +
  theme_bw() + 
  theme(legend.position = 'right') +
  ggtitle('Gene Ontology Enrichment Analysis (High vs Low risk group)')


#pdf(file.path(params$outputpath,paste0('FigureS2_DEG.pdf')),width = 8,height = 8)
volcano_plot / go_plot + plot_annotation(tag_levels = 'A')
#dev.off()
